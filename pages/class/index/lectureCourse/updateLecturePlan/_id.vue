                ...
                <div class="date_area">
                  ...
                  <span class="box01 box_padding">
                    ...
                    >
                      ...
                      {{ isStartTime ? '오전' : '오후' }}
                      {{ setTimeFormat(syllabus.lep_time_stime) }} -
                      {{ isEndTime ? '오전' : '오후' }}
                      {{ setTimeFormat(syllabus.lep_time_etime) }}
                    </span>
...
export default {
  ...
  methods: {
    ...
    async getSyllabus() {
      const payload = {
        lec_idx: this.lectureInfo.lectureIdx,
        lep_idx: this.$route.query.lepidx,
      }
      await apiLectureCourse
        .getSyllabus(payload)
        .then(({ data: { data } }) => {
          this.syllabus = data
          this.syllabus.lep_time_stime = this.setStartTimeFormat(
            data.lep_time_stime
          )
          this.syllabus.lep_time_etime = this.setEndTimeFormat(
            data.lep_time_etime
          )
        })
        .catch((err) => {
