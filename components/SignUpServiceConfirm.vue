  ...
  <div class="right_section">
    <div id="loginRwrap" class="wrap">
      ...
      <div class="agree_section">
        <div class="title">
          <CustomCheckBox @toggle-checkbox="toggleCheckboxEvent" :checkBoxData="checkboxDatas[0]"/>
        </div>
        <div class="cnts">
          <CustomCheckBox @toggle-checkbox="toggleCheckboxEvent" :checkBoxData="checkboxDatas[1]"/>
          <CustomCheckBox @toggle-checkbox="toggleCheckboxEvent" :checkBoxData="checkboxDatas[2]"/>
        </div>
      </div>
      <CustomNextBtn @nextBtn-click='nextBtnEvent' :btnAbled='nextBtnable'/>
      <div class="question_txt"><NuxtLink to="/">이미 계정이 있으신가요?</NuxtLink></div>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const checkboxDatas = ref([
    ...
    {title:'(필수) 서비스 이용약관',checked:false,id:'checkBox_1',targetModal:'#modalTerms01'},
    {title:'(필수) 개인정보 처리방침',checked:false,id:'checkBox_2',targetModal:'#modalTerms02'}
  ]);
